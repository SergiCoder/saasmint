...
Defines the shape of SaaSmint as a single git monorepo: the Django backend under `core/`, the Next.js frontend under `app/`, shared infrastructure under a root `infra/`, and OpenSpec artifacts under `openspec/`. It also fixes the migration invariants that hold once the two predecessor repositories are merged — preserved commit history, a single continuous version line, and a consolidated VS Code workspace for running and debugging both stacks locally.
